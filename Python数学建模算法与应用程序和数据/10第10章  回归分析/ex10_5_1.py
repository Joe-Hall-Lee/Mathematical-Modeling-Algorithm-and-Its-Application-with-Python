...
yb = Y.mean()  # 计算 y 的观测值的平均值
yh = XX @ cs  # 计算 y 的估计值
q = sum((yh-Y)**2)  # 计算残差平方和
u = sum((yh-yb)**2)  # 计算回归平方和
m = 3
n = len(Y)  # 变量个数和样本容量
F = u/m/(q/(n-m-1))  # 计算 F 统计量的值
print('F =', round(F, 4))
fw = f.ppf(0.95, m, n-m-1)  # 计算上 alpha 分位数
print('F 分布的上 alpha 分位数：', round(fw, 4))
c = np.diag(np.linalg.inv(XX.T @ XX))
ts = cs/np.sqrt(c)/np.sqrt(q/(n-m-1))  # 计算 t 统计量的值
tw = t.ppf(0.975, n-m-1)  # 计算上 alpha / 2 分位数
print('t 统计量值为：', np.round(ts, 4))
print('t 分布的上 alpha / 2 分位数：', round(tw, 4))
XD = np.delete(XX, 1, axis=1)  # 删除 x1 的观测值
cs2 = np.linalg.pinv(XD) @ Y  # 重新拟合参数
print('x2, x3 模型的参数值：', np.round(cs2, 4))
