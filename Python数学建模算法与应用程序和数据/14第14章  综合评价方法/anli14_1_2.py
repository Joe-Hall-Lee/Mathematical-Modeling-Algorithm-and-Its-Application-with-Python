...
def f1(t): return [1/(1+t[0]/(1-t[1])**2)-0.01,
                   1/(1+t[0]/(4-t[1])**2)-0.8]


c1 = fsolve(f1, [0.5, 0.5])  # 待定参数 alpha, beta
def f2(t): return [t[0]*np.log(4)+t[1]-0.8, t[0]*np.log(7)+t[1]-1]


c2 = fsolve(f2, [0.5, 0.5])  # 待定参数 a, b


def f(x): return (1/(1+c1[0]/(x-c1[1])**2) * ((x >= 1) & (x <= 4)) +
                  (c2[0]*np.log(x)+c2[1]) * ((x > 4) & (x <= 7)))


f17 = f(np.arange(1, 8))  # 计算对应的函数值
d1 = np.loadtxt('anli14_1_1.txt')
d2 = np.loadtxt('anli14_1_3.txt')
a = d1[:, 3:]
b = d2[:, 6:]


def g(x): return (4*(x == 0)+5*(x == -1)+6*(x == -2)+7*(x == -3) +
                  3*(x == 1)+2*(x == 2)+(x >= 3))
